...
// =============================================================================
// Regression tests for multiline value parsing (issue #3)
// =============================================================================

test "regression: Google service account key parsing" {
    ...
    // Verify the entire JSON structure is captured
...
test "regression: multiline value followed by other vars" {
    // Ensure variables after multiline values are still parsed correctly
    const allocator = std.testing.allocator;
    var store = EnvStore.init(allocator);
    defer store.deinit();
...
test "regression: deeply nested JSON structure" {
    const allocator = std.testing.allocator;
    var store = EnvStore.init(allocator);
    defer store.deinit();
...
test "regression: value with embedded newlines as escape sequences" {
    const allocator = std.testing.allocator;
    var store = EnvStore.init(allocator);
    defer store.deinit();

    // This tests the \n inside the private_key which should become actual newlines
    const content =
...
test "regression: mixed quote styles in same file" {
    const allocator = std.testing.allocator;
    var store = EnvStore.init(allocator);
    defer store.deinit();
...
test "regression: AWS credentials JSON format" {
    const allocator = std.testing.allocator;
    var store = EnvStore.init(allocator);
    defer store.deinit();
...
test "regression: Firebase service account format" {
    const allocator = std.testing.allocator;
    var store = EnvStore.init(allocator);
    defer store.deinit();
...
test "regression: multiline in MultiEnvStore with multiple files" {
    const allocator = std.testing.allocator;
    var store = MultiEnvStore.init(allocator);
    defer store.deinit();
    ...
    try parseEnvContentMulti(dev_content, ".env", &store);
    try parseEnvContentMulti(prod_content, ".env.production", &store);
...
test "regression: unclosed quote at end of file" {
    // Edge case: what happens if quote is never closed?
    const allocator = std.testing.allocator;
    var store = EnvStore.init(allocator);
    defer store.deinit();
    ...
    // Should still capture the value (everything until EOF)
    const value = store.get("KEY").?.value;
    try std.testing.expectEqualStrings("unclosed value", value);
}

test "regression: empty multiline JSON" {
    const allocator = std.testing.allocator;
    var store = EnvStore.init(allocator);
    defer store.deinit();
...
test "regression: JSON array value" {
    const allocator = std.testing.allocator;
    var store = EnvStore.init(allocator);
    defer store.deinit();
